...
func (h *RiskHandler) GetMetrics(c *gin.Context) {
	...
	sessions, sessErr := h.db.ListActiveSessions(ctx)
	if sessErr != nil {
		log.Warn().Err(sessErr).Msg("risk/GetMetrics: failed to fetch sessions for VaR scaling, falling back to totalExposure")
	}
